...
After loading the main code file, we import the relevant packages as mentioned in the notebook in the initial part.  
We load the brown university data on editorials as selected in the brown corpus of the NLTK package. 
Using the same LDA approach based on BOW or TF-IDF, we can generate a dataframe of the sentences, their probability scores for each topic, their dominant topic, and the corresponding name of the topic as a result.
The whole pipeline can be run through a single execution run over the entire code notebook. The whole notebook has no external dependency for dataset or anything as of currently. But, if there's another dataset required for generating the topics, the same can be developed and tweaked using a similar code line.
# Conclusion

After evaluating the topics of the words generated by LDA, I could get 6 topics as follows:

Development & Industry -  Having mentions of the development and Industry impact-related themes.
General Debates of People - Having themes of general discussion or talks regarding people and by people.
Governments & Nations - Having themes of government activities and nations.
People & Political Parties - Having themes of political parties and people impacted due to their activity.
Presidents and Politics - Themes related to people at esteemed legislative positions.
War & Power - Themes mentioning war and the use of power to drive or motivate them.
